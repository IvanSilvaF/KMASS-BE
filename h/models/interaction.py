...
class Interaction(Base):
    """Model class representing a single interaction."""
    ...
    #: The timestamp when the interaction was send.
    request_time = sa.Column(
        sa.DateTime,
        # default=datetime.datetime.utcnow,
        # server_default=sa.func.now(),
        # nullable=False,
    ...
    #: The timestamp when the interaction was received.
    response_time = sa.Column(
        sa.DateTime,
        # default=datetime.datetime.utcnow,
        # server_default=sa.func.now(),
        # nullable=False,
    ...
    cookie_session = sa.Column(sa.UnicodeText, nullable=False)
    cookie_auth = sa.Column(sa.UnicodeText)

    action_type = sa.Column(sa.Integer)
    action_name = sa.Column(sa.UnicodeText)
    ...
    _md5 = sa.Column("md5", sa.UnicodeText)
    ...
    document = sa.orm.relationship("Document", backref="interactions")
