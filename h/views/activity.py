...
from h.models.document import _document
from h.activity import query, bucketing
from h.traversal import user
...
def match_words(p, t):
    ...
    for i in range(len(t)):
        ...
        j = 0
        while j < len(p):
            if each_index < len(t) and p[j] == t[each_index]:
                each_index += 1
                j += 1
                if j >= max_match:
                    max_match = j
                    # print(max_match, each_index, j)
                    # print(t[each_index])
                    if each_index == len(t):
                        if j == len(p):
                            is_total_match = True
                        break
                    if max_match == len(p) and t[each_index].isspace():
                        is_total_match = True

            else:
                break

    return max_match, is_total_match
...
class SearchController:
    ...
    def __init__(self, request):
        ...
        self.logout_redirect = self.request.route_url("login")
    ...
    def search(self):
        ...
        query_list = query_params.getall('any')
        raw_score = {}
        result_docs = []
        udocs=[]
        documents=[]
        ...
        if len(query_list) > 0:
            print("search 3")
            # search public and private files
            # documents = query.fetch_documents(self.request.db, query_list[0])
            # public_docs = self.request.db.query(models.Document).all()
            test = self.request.db.query(models.Test).all()
            print(test[0].name)

            private_docs = self.request.db.query(models.UserDoc) \
                .filter((models.UserDoc.userid == 'public') | (models.UserDoc.userid == username)).all()

            # for index, item in enumerate(query_list):
            #     key_word = item
            #     for doc in public_docs:
            #         # key_word is match doc.title
            #         if doc.title and key_word.lower() in doc.title.lower():
            #             if doc not in documents: # TODO
            #                 documents.append(doc) # TODO

            weights = {}
            min_word = 100
            sum_length = len(''.join(query_list))
            for word in query_list:
                if len(word) < min_word:
                    min_word = len(word)
                weights[word] = len(word)/sum_length

            # print(weights)

            for udoc in private_docs:
                for word in query_list:
                    if udoc.title:
                        grade = 0
                        match_result = match_words(word.lower(), udoc.title.lower())
                        if match_result[1] is True:
                            grade = 1 + 1 / len(query_list)
                        else: # match_result[1] is False
                            if len(word) == match_result[0]:
                                ret = match_result[0] / len(word)
                                if len(word) > 5:
                                    grade = ret * 0.95 + 1 / len(query_list)
                                elif len(word) == 5:
                                    grade = ret * 0.9
                                elif len(word) == 4:
                                    grade = ret * 0.8
                                ...
                                ret = match_result[0] / len(word)
                                if match_result[0] > 5:
                                    grade = ret * 0.85 + 1 / len(query_list)
                    ...
                    # if grade > 0.03:
                    if udoc in raw_score:
                        raw_score[udoc] += grade * weights[word]
                    else:
                        raw_score[udoc] = grade * weights[word]


                    # if udoc.title and word.lower() in udoc.title.lower():
                    #     if udoc in raw_score:
                    #         raw_score[udoc] += len(word)
                    #     else:
                    #         raw_score[udoc] = len(word)

            # for item in raw_score:
            #     raw_score[item] = raw_score[item] / len(item.title)

            order_dict = {k: v for k, v in sorted(raw_score.items(), key=lambda item: item[1], reverse=True)}

            for item in order_dict:
                if order_dict[item] < 0.177:
                   continue
                print("order list", item.title, order_dict[item])

                data = {
                        'title': item.title,
                        'web_uri':item.target_uri
                    }
                _doc = _document.Document(**data)
                if _doc not in documents:
                    documents.append(_doc) # TODO
            ...
            #         data = {
            #             'title': udoc.title,
            #             'web_uri':udoc.target_uri
            #         }
            #         _doc = _document.Document(**data)
            #         if _doc.title and key_word.lower() in _doc.title.lower():
            #             if _doc not in documents: # TODO
            #                 result_docs.append(_doc)
            #                 documents.append(_doc) # TODO
        ...
        tf_list = []
        document_buckets_list = []

        for item in documents:
            tf = bucketing.Timeframe('', '')
            # documentbucket was created by add() function
            tf.add(item)
            tf_list.append(tf)

        for tf in tf_list:
            document_buckets_list += list(tf.document_buckets.values())

        if len(query_list) == 0:
            results = query.ActivityResults(total=0,
            aggregations={'tags': [{'tag': 'company', 'count': 1}, {'tag': 'test', 'count': 1}]},
            timeframes=[]
            )
            pass
        else: # test
            results = query.ActivityResults(total=len(documents),
            aggregations={'tags': [{'tag': 'company', 'count': 1}, {'tag': 'test', 'count': 1}]},
            # type <class 'h.models.annotation.Annotation'>
            timeframes=tf_list,
            )
            pass
        ...
        if len(query_list) > 0:
            record2 = {
                ...
                'cookie_session': self.request.cookies.get('session'),
                'cookie_auth': self.request.cookies.get('auth'),
                'action_type': 7,
                'action_name': 'query submit response',
                'event_type': 4,
                'text': ' '.join(query_list),
            }
            storage.create_interaction(self.request, record2, 1)
        ...
        return {
            ...
            "page_result": document_buckets_list,
            "num_each_page": PAGE_SIZE,
...
@view_defaults(
    route_name="activity.file_manager", renderer="h:templates/activity/file_manager.html.jinja2"
)
class FileController:
    ...
    def search(self):
        ...
        res = self.request.db.query(models.UserDoc).filter(
            models.UserDoc.userid==self.request.user.username
        ).all()
        ...
        # results = ActivityResults(total=18, aggregations={'tags': [{'tag': 'company', 'count': 1}, {'tag': 'test', 'count': 1}]},
        #     timeframes=[])

        results = {'total':len(res), 'aggregations':{'tags': [{'tag': 'company', 'count': 1}, {'tag': 'test', 'count': 1}]}, 'timeframes':[],
            'documents': doc_list
        }
        ...
        return {
            ...
            "username_from_id": username_from_id,
            # The message that is shown (only) if there's no search results.
            "zero_message": _("No resources matched your search."),
        }
