...
def _login_redirect_url(request):
    return request.route_url("activity.search", username=request.user.username)
...
class AuthController:
    ...
    def post(self):
        ...
        # catch event
        print("1 request.cookies", self.request.response.headers.get('Set-Cookie'))
        ...
        record = {
            ...
            'action_type': 1,
            'action_name': 'login click',
            'event_type': 1,
        }
        # TODO set cookies
        # self.request.response.unset_cookie('auth')

        storage.create_interaction(self.request, record, 1)
    ...
    def logout(self):
        ...
        record = {
            ...
            'cookie_auth': self.request.cookies.get('auth'),
            'action_type': 2,
            'action_name': 'logout click',
            'event_type': 1, # click event
        }
        # # apprecord = json.dumps(record)
        storage.create_interaction(self.request, record, 2)
