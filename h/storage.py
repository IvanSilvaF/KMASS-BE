...
def fetch_documents_by_title(session, word):
    """
    https://flask-sqlalchemy.palletsprojects.com/en/2.x/queries/#querying-records
    """
    search_word = '{}%'.format(word)
    result = session.query(models.Document).filter(models.Document.title.startswith(word)).all()# .filter(models.Document.title.like('%'))
    return result
...
def create_interaction(request, data, type):
    """
    Create an interaction record from already-validated data.
    ...
    :returns: the created and flushed interaction record

    """
    interaction = models.Interaction(**data)

    # interaction.document = update_document_metadata(
    #     request.db,
