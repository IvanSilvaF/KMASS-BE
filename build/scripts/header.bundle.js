...
class EnvironmentFlags {
  ...
  init(url) {
    var JS_LOAD_TIMEOUT = 5000; // Mark browser as JS capable

    this.set('js-capable', true); // Set a flag to indicate touch support. Useful for browsers that do not
    // support interaction media queries.
    // See http://caniuse.com/#feat=css-media-interaction

    this.set('touch', this._element.ontouchstart); // Set an additional flag if scripts fail to load in a reasonable period of
    // time

    this._jsLoadTimeout = setTimeout(() => {
      this.set('js-timeout', true);
    }, JS_LOAD_TIMEOUT); // Process flag overrides specified in URL
...
window.envFlags.init(); // Set up the Google Analytics command queue if we have a tracking ID.
